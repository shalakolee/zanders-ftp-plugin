<?php
/**
 * Plugin Name: Zanders FTP Products
 * Plugin URI: http://git.shalakolee.com/
 * Description: Download Inventory from Zanders ftp and sync with woocommerce products
 * Version: 0.1a
 * Author: Shalako Lee
 * Author URI: http://git.shalakolee.com
 * License: GPL2
 */

/**
 * a few notes...
 *		wordpress only runs cron jobs when a page is accessed so it is better to set a system cron if you can.
 *			to set a system cron:
 *			define('DISABLE_WP_CRON', true); in wp.config
 *			add this to the cron jobs: 15 * * * * wget -q -O – http://parabellumTDS.561dev.com/wp-cron.php?doing_wp_cron
 *			this will have the cron request the page every 15 minutes
 */
session_start();

//forbid inet users
if (!defined('ABSPATH')):
	header('HTTP/1.0 403 Forbidden');
exit;


endif;
//include the ftpconnection class
if(!class_exists("shalako_ftpConnection")):
	include dirname(__FILE__) .'/classes/ftpConnection.php';
endif;



//############################ CHECK OPTIONS ######################################

function option_enabled( $option, $default=false ){
	global $plugin_settings;

	$option = get_option( $option );
    if( $option === false): //see if this is the first time loading, if it is, the option is not on - return false
    	if( $default == true):// want this on by default
    		return true;
		else:
	        return false;
    	endif;
    else:
        if( $option != "" ): // option is not blank, return true
            return true;
        else: // the option has no value, return false
            return false;
        endif;
     endif;
}
//###################################### CUSTOM FORM HANDLER ###########################################
add_action( 'admin_post_update_custom_settings', 'admin_update_custom_settings' );
function admin_update_custom_settings() {
	
	//check commands and find out which one to run
	// handle the general settings 
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'update_general_settings' && wp_verify_nonce( $_POST['check_ftp'], 'check_ftp' ) ):

	  	//set the zanders ftp options
	  	$message .= update_option('zanders_ftp_server',isset($_POST['zanders_ftp_server']) ? $_POST['zanders_ftp_server'] : '') ? "<p><strong>Updated Server URL</strong></p>" : "" ;
	  	$message .= update_option('zanders_ftp_port',isset($_POST['zanders_ftp_port']) ? $_POST['zanders_ftp_port'] : '') ? "<p><strong>Updated Server Port</strong></p>" : "" ;
	  	$message .= update_option('zanders_ftp_username',isset($_POST['zanders_ftp_server']) ? $_POST['zanders_ftp_username'] : '') ? "<p><strong>Updated Username</strong></p>" : "" ;
	  	$message .= update_option('zanders_ftp_password',isset($_POST['zanders_ftp_password']) ? $_POST['zanders_ftp_password'] : '') ? "<p><strong>Updated Password</strong></p>" : "" ;

	  	if($message):
	  		$_SESSION['message'] .= "<div class='updated dismissable'>";
	  		$_SESSION['message'] .= $message;
	  		$_SESSION['message'] .= "</div>";

  		endif;

		header('Location: ' . $_REQUEST['returl'], true, 301);
		die();
	endif;
	//handle the file settings
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'update_download_files' && wp_verify_nonce( $_POST['check_download_files'], 'check_download_files' ) ):

	  	//set the zanders ftp options
	  	$message .= update_option('zanders_ftp_local_folder',isset($_POST['zanders_ftp_local_folder']) ? $_POST['zanders_ftp_local_folder'] : '') ? "<p><strong>Updated Local Directory</strong></p>" : "" ;
	  	$message .= update_option('zanders_ftp_remote_folder',isset($_POST['zanders_ftp_remote_folder']) ? $_POST['zanders_ftp_remote_folder'] : '') ? "<p><strong>Updated Remote Directory</strong></p>" : "" ;

	  	if($message):
	  		$_SESSION['message'] .= "<div class='updated dismissable'>";
	  		$_SESSION['message'] .= $message;
	  		$_SESSION['message'] .= "</div>";
  		endif;

		header('Location: ' . $_REQUEST['returl'], true, 301);
		die();
	endif;	

}
//########################################### AJAX ##############################################

add_action( 'wp_ajax_check_ftp', 'checkftp' );
function checkftp() {
	error_reporting(0);
	$server = $_REQUEST['server'];
	$port = $_REQUEST['port'];
	$username = $_REQUEST['username'];
	$password = $_REQUEST['password'];

	$myconn = new shalako_FtpConnection($server, $username, $password, true, $port, 10 );
	$connstatus = $myconn->connect(); 

	if($connstatus): // we are connected to the server and logged in
		echo "true";
	else:
		echo "false";
	endif;
		$myconn->closeconnection();

	wp_die(); 
}
add_action( 'wp_ajax_list_remote_files', 'list_remote_files' );
function list_remote_files(){
	error_reporting(0);
	$server 	= get_option("zanders_ftp_server");
	$port 		= get_option("zanders_ftp_port");
	$username 	= get_option("zanders_ftp_username");
	$password 	= get_option("zanders_ftp_password");
	$remotefolder = get_option("zanders_ftp_remote_folder");
	if($remotefolder):
		$remotefolder = strtolower($remotefolder);
	endif;


	$myconn = new shalako_FtpConnection($server, $username, $password, true, $port, 10 );
	$connstatus = $myconn->connect(); 

	if($connstatus): // we are connected to the server and logged in
		//get a list of files and return them 
		if($remotefolder):
			echo json_encode($myconn->listdir($remotefolder));
		else:
			echo json_encode($myconn->listdir("/inventory/"));
		endif;
	else:
		echo "false";
	endif;
	$myconn->closeconnection();

	wp_die(); 
}
add_action( 'wp_ajax_update_filelist', 'update_filelist' );
function update_filelist(){
	$result = update_option("zanders_file_list", $_POST['filelist']);
	echo json_encode(get_option("zanders_file_list"));
	wp_die(); 
}
add_action( 'wp_ajax_download_file', 'download_file' );
function download_file(){

	error_reporting(0);
	$server 	= get_option("zanders_ftp_server");
	$port 		= get_option("zanders_ftp_port");
	$username 	= get_option("zanders_ftp_username");
	$password 	= get_option("zanders_ftp_password");
	$remotefolder = get_option("zanders_ftp_remote_folder");
	$localfolder = get_option("zanders_ftp_local_folder");

	$file = $_POST['file'];

	if($remotefolder):
		$remotefolder = strtolower($remotefolder);
	else:
		$remotefolder = '/inventory/';
	endif;

	if(!$localfolder):
		$localfolder = $_SERVER['DOCUMENT_ROOT'] . "/wp-content/uploads/gzandersftp/";
	endif;

	$myconn = new shalako_FtpConnection($server, $username, $password, true, $port, 10 );
	//set folcers for the download
	$myconn->remotefolder = $remotefolder;
	$myconn->localfolder = $localfolder;

	$connstatus = $myconn->connect(); 
	$filestatus = $myconn->downloadfile($file) ? "{$file} download success <br />" : "{$file} did not download <br />";
	echo $filestatus;

	wp_die(); 
}

add_action( 'wp_ajax_activate_zanders_cron', 'activate_zanders_cron' );
function activate_zanders_cron(){
	//Use wp_next_scheduled to check if the event is already scheduled
	$timestamp = wp_next_scheduled( 'zanders_ftp_download_inventory' );
	$status = false;
	//If $timestamp == false schedule daily backups since it hasn't been done previously
	if( $timestamp == false ){
	$status = wp_schedule_event( time(), 'quarterhour', 'zanders_ftp_download_inventory' );
	}

	wp_die(); 
}
add_action( 'wp_ajax_deactivate_zanders_cron', 'deactivate_zanders_cron' );
function deactivate_zanders_cron(){
	$hookstatus = wp_clear_scheduled_hook('zanders_ftp_download_inventory');
	
	wp_die(); 
}


//############################## CRON SETTINGS ############################

function zanders_cron_enabled(){
	$timestamp = wp_next_scheduled( 'zanders_ftp_download_inventory' );	

	if($timestamp):
		return true;
	else:
		return false;
	endif;
}

register_deactivation_hook(__FILE__, 'deactivate_zanders_cron'); // remove cron when plugin is not active

//########## ADD CUSTOM SCHEDULE #################
add_filter( 'cron_schedules', 'sl_add_15_min_schedule' ); 
function sl_add_15_min_schedule( $schedules ) {
  $schedules['quarterhour'] = array(
    'interval' => 900, //15 minutes, this value is is seconds
    'display' => __( 'Every 15 Minutes', 'zanders-ftp' )
  );
  return $schedules;
}
//no longer using this, will remove soon
// register_activation_hook( __FILE__, 'sl_create_download_schedule' );
// function sl_create_download_schedule(){
//   //Use wp_next_scheduled to check if the event is already scheduled
//   $timestamp = wp_next_scheduled( 'zanders_ftp_download_inventory' );

//   //If $timestamp == false schedule daily backups since it hasn't been done previously
//   if( $timestamp == false ){
//     wp_schedule_event( time(), 'quarterhour', 'zanders_ftp_download_inventory' );
//   }
// }

add_action( 'zanders_ftp_download_inventory', 'sl_download_files' );
function sl_download_files(){
	//lets make the ftp connection and download the files
	error_reporting(-1); //turn on error reporting here only
	set_time_limit(0); //make sure script load time is off
	global $woocommerce;
	$timerstart = microtime(true); //start the function timer
	//log file
	$logfile = plugin_dir_path( __FILE__ ) . "/log.txt";

	$logdata = '';
    $logdata .= '////////////////////////////////// '. date("Y/m/d") . " - " .date("h:i:sa") . "//////////////////////////////////\n";

	$server 	= get_option("zanders_ftp_server");
	$port 		= get_option("zanders_ftp_port");
	$username 	= get_option("zanders_ftp_username");
	$password 	= get_option("zanders_ftp_password");
	$remotefolder = get_option("zanders_ftp_remote_folder");
	$localfolder = get_option("zanders_ftp_local_folder");
	$filelist 	= get_option('zanders_file_list');

	//set default folder paths if there are not any specified
	$remotefolder ? $remotefolder = strtolower($remotefolder) : $remotefolder = '/inventory/';
	!$localfolder ? $localfolder = $_SERVER['DOCUMENT_ROOT'] . "/wp-content/uploads/gzandersftp/" : "";

	//if there is a file list to download then lets download them
	if($filelist):
		$myconn = new shalako_FtpConnection($server, $username, $password, true, $port, 10 );
		$myconn->remotefolder = $remotefolder;
		$myconn->localfolder = $localfolder;
		$myconn->filelist = $filelist;

		if($myconn->connect()):
			foreach($myconn->filelist as $file):
				$downloadStart = microtime(true);
				$status = $myconn->downloadfile($file);
				$downloadTime = number_format((microtime(true)-$downloadStart),4);
				if($status):
					$logdata .= date("Y/m/d"). " - " .date("h:i:sa") . " - Sucessfully downloaded {$file} in {$downloadTime} seconds \n";
				else:
					$logdata .= date("Y/m/d"). " - " .date("h:i:sa") . " - Failed to download {$file} in {$downloadTime} seconds \n";
				endif;
			endforeach;
		else:
			$logdata .= date("Y/m/d"). " - " .date("h:i:sa") . " - Failed to Connect to FTP Server! \n";

		endif;
	endif;

	$myxml = load_xmldata($localfolder."liveinv.xml");
	$myxmlobject = array();

	foreach($myxml as $xmlnode):
		$itemno = (string)$xmlnode->ITEMNO;
		$qty = (string)$xmlnode->AVAILABLE;
		$price = (string)$xmlnode->ITEMQTYPRICING->ITEMPRICE;
		$myxmlobject[$itemno]['price'] = $price;
		$myxmlobject[$itemno]['qty'] = $qty;
	endforeach;

	//get array of sku ready for sql 
	$myxmlarray = array_map(function($item) {
		return "'".$item."'";
	}, array_keys($myxmlobject));
	//get list of products that match data from the xml
	$myproducts = get_products_array($myxmlarray);
	$changedrecordcounter = 0;
	foreach($myproducts as $product):
		//print_r($product); //debuggins
		$product_id = $product->post_id;
		$sku = $product->meta_value;
		$updatedprice = $myxmlobject[$sku]['price'];
		$updatedqty = $myxmlobject[$sku]['qty'];

		$myproduct = new WC_Product($product_id); // get the woocommerce product
		//############################## UPDATING PRICING   ##############################
		$currentprice =  get_post_meta($product_id, "_regular_price", true);
		if($updatedprice != $currentprice ):
			$result = update_post_meta($product_id, '_regular_price', $updatedprice);
			if($result):
				$changedrecordcounter++;
				$logdata .= date("Y/m/d"). " - " .date("h:i:sa") . " - updated product #" . $myproduct->id . " price from $" .$currentprice . " to $" . $updatedprice . "\n";
			endif;
		endif;

		//############################## UPDATING INVENTORY ##############################

		//###### update the stock quantity ########//
		if($updatedqty != $myproduct->stock):
			//also need to set the stock status here   (instock || outofstock)
			$newqty = update_post_meta($product_id, '_stock', $updatedqty);
			if($newqty):
				$changedrecordcounter++;
				$logdata .= date("Y/m/d"). " - " .date("h:i:sa") ." - updated product #" . $myproduct->id . " stock from " .$myproduct->stock . " to " . $updatedqty . "\n";
			endif;
		endif;

		//###### update the stock status  ########//
		$stockstatus = get_post_meta($product_id, "_stock_status", true);

		if($updatedqty > 0): //new stock is instock
			if($stockstatus == 'outofstock'):
				$newstockstatus = update_post_meta($product_id, "_stock_status", "instock");
			else:
				$newstockstatus = false;
			endif;

			if($newstockstatus):
				$changedrecordcounter++;
				$logdata .= date("Y/m/d"). " - " .date("h:i:sa") ." - changed product #{$product_id} stock status from {$stockstatus} to in stock \n";
			endif;
		else: //outofstock
			if($stockstatus = 'instock'):
				$newstockstatus = update_post_meta($product_id, "_stock_status", "outofstock"); 
			else: 
				$newstockstatus = false;
			endif;
			if($newstockstatus):
				$changedrecordcounter++;
				$logdata .= date("Y/m/d"). " - " .date("h:i:sa") ." - changed product #{$product_id} stock status from {$stockstatus} to out of stock \n";
			endif;
		endif;

	endforeach;

	$totalfunctiontime = microtime(true) - $timerstart;

    $logdata .= '//////////////// Processed ' . count($myproducts) .' products ('.$changedrecordcounter.' modified) in '. number_format($totalfunctiontime,4) . " seconds /////////////////\n\n\n\n";
	//write the log data to the file	
	file_put_contents($logfile, $logdata, FILE_APPEND | LOCK_EX);
	//set the xml to nothing
	$myxml = null;
}

//############################## RETRIEVE WOOCOMMERCE PRODUCT ARRAY ##############################
function get_products_array( $sku ) {
  global $wpdb;
  $sku = implode(',' , $sku);
  $products = $wpdb->get_results( "SELECT post_id, meta_value FROM $wpdb->postmeta WHERE meta_key='_sku' AND meta_value IN ({$sku}) ");
  if ( $products ) return $products;
  return null;
}
//############################## LOAD XML DATA  ##############################
function load_xmldata($file){
if(!$file){
	die("no file specified");
}
if(!file_exists($file)){
	die("file not found in filesystem");
}
$xml=simplexml_load_file($file, "SimpleXMLElement", LIBXML_NOCDATA) or die("Error: Cannot create object");
return $xml;
}

//############################## ADMIN MENU ######################################

//add menu page
add_action('admin_menu', '_add_custom_menu_pages',99);
function _add_custom_menu_pages(){
	add_menu_page("Zanders FTP", "Zanders FTP", 'manage_options', "zanders", '_include_custom_main_page','');
}
function _include_custom_main_page(){
	// output the admin page
	?>

	<?php //since this is the main page, we are going to check the ftp connection on load to make sure the user
		// has entered correct information, we are going to hide the tabs and redirect them to general settings tab if they have not
	// setting this to use the session so we dont keep hitting the ftp server.
		if(!isset($_SESSION['correct_ftp']) || $_SESSION['correct_ftp'] == 'false'):

			error_reporting(0);
			$server 	= get_option("zanders_ftp_server");
			$port 		= get_option("zanders_ftp_port");
			$username 	= get_option("zanders_ftp_username");
			$password 	= get_option("zanders_ftp_password");

			$myconn = new shalako_FtpConnection($server, $username, $password, true, $port, 10 );
			$connstatus = $myconn->connect(); 
			if($connstatus): // we are connected to the server and logged in
				$_SESSION['correct_ftp'] = 'true';
			else:
				$_SESSION['correct_ftp'] = 'false';
			endif;
			$myconn->closeconnection();
		endif;
		$correct_ftp = $_SESSION['correct_ftp'];
		
		//$correct_ftp = 'true'; //debugging
		
		if($correct_ftp === 'false' && $_GET['tab'] != "general_settings"):
			header("Location: ?page=zanders&tab=general_settings");
		endif;
		error_reporting(-1);
	?>
		<script>
			jQuery(document).ready(function($){
				$(".dismissable").click(function(){
					$(this).hide();
				});
			});
		</script>
		<style>
		.overlay, .overlay1{
		    background: url("data:image/gif;base64,") center center no-repeat rgba(255,255,255,0.7) !important;
			  position:absolute;width:100%;height:100%;z-index:10;display:none;top:0px;left:0px;
		}
		input[type="text"], input[type="password"]{width:500px !important;}
		</style>


	<div class="wrap">
		<?php  echo "<h2>Zanders FTP </h2>"; ?>

		<?php // ################# TESTING ######################  ?>
		<?php //sl_download_files(); ?>


		<?php
		$active_tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : 'general_settings';
		?>
		<h2 class="nav-tab-wrapper">
			<a href="?page=zanders&tab=general_settings" class="nav-tab <?php echo $active_tab == 'general_settings' ? 'nav-tab-active' : ''; ?>">General Settings</a>
			<?php if($correct_ftp === 'true'): ?>
			<a href="?page=zanders&tab=download_files" class="nav-tab <?php echo $active_tab == 'download_files' ? 'nav-tab-active' : ''; ?>">Download Files</a>
			<!-- <a href="?page=zanders&tab=import_products" class="nav-tab <?php echo $active_tab == 'import_products' ? 'nav-tab-active' : ''; ?>">Import Products</a> -->
			<!-- <a href="?page=zanders&tab=cron_settings" class="nav-tab <?php echo $active_tab == 'cron_settings' ? 'nav-tab-active' : ''; ?>">Cron Settings</a> -->
			<?php endif; ?>
		</h2>
		<?php if($active_tab == 'general_settings'): ?>
			<?php 
			if (isset($_SESSION['message'])) {
		    echo $_SESSION['message']; //display the message
		    unset($_SESSION['message']); //free it up
		}
		?>
		<script>
			jQuery(document).ready(function($){

				//disable the submit button until we have checked the username and password
				$('#submit').prop("disabled",true);
				$('#submit').hide();
			    $("#checkftp").click(function(){
			    	$('.overlay').fadeIn();
	                jQuery.ajax({
		                url: "<?php echo admin_url('admin-ajax.php'); ?>",
		                type: 'POST',
		                data: {
		                    action: 'check_ftp',
		                    server: $("#zanders_ftp_server").val(),
		                    port: $("#zanders_ftp_port").val(),
		                    username: $("#zanders_ftp_username").val(),
		                    password: $("#zanders_ftp_password").val()
		                },
		                dataType: 'html',
		                success: function(response) {
		                    //console.log(response);
		                    if(response == "true"){
            					$('#submit').prop("disabled",false);
								$('#submit').show();
            					//console.log("true")
		                    }else{
            					$('#submit').prop("disabled",true);
								$('#submit').hide();
								alert("There was an error connecting")
		                    }
		                    $(".overlay").fadeOut();
		                }
			    	});
	            });
            });
		</script>

		<form name="general_settings_form" method="post" action="/wp-admin/admin-post.php">

			<h2>FTP Connection Options</h2>
			<?php 
			if(!$correct_ftp): 
				echo "<div class='error dismissable'><p>Please Enter Valid FTP Credentials</p></div>";
			endif;
			?>

			<div class="mytable" style="position:relative;">
			<div class="overlay" style="position:absolute;width:100%;height:100%;z-index:10;display:none;"></div>
			<table class="form-table">
				<tr valign="top">
					<th scope="row">FTP Server</th>
					<td>
						<input type="text" name="zanders_ftp_server" id="zanders_ftp_server" value="<?php echo get_option("zanders_ftp_server"); ?>">
						<em></em>
					</td>
				</tr>       
		        <tr valign="top">
		        	<th scope="row">FTP Port</th>
		        	<td>
		        		<input type="text" name="zanders_ftp_port" id="zanders_ftp_port" value="<?php echo get_option("zanders_ftp_port"); ?>" placeholder="21">
		        		<em></em>
		        	</td>
		        </tr>				
		        <tr valign="top">
					<th scope="row">FTP Username</th>
					<td>
						<input type="text" name="zanders_ftp_username" id="zanders_ftp_username" value="<?php echo get_option("zanders_ftp_username"); ?>">
						<em></em>
					</td>
				</tr>        
				<tr valign="top">
					<th scope="row">FTP Password</th>
					<td>
						<input type="password" name="zanders_ftp_password" id="zanders_ftp_password" value="<?php echo get_option("zanders_ftp_password"); ?>">
						<em></em>
					</td>
				</tr>

			</table>
			</div>
			<input type="button" id="checkftp" value="Test Connection">
		 <?php wp_nonce_field( 'check_ftp', 'check_ftp' ); ?>
		<input type="hidden" name="action" value="update_custom_settings">
		<input type="hidden" name="command" value="update_general_settings">
		<input type="hidden" name="returl" value="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">


		<?php submit_button( 'Update Settings', 'primary', 'submit' ) ?>
	</form>

	<?php  elseif($active_tab == 'download_files'): ?>
		<?php 
		if (isset($_SESSION['message'])) {
		    echo $_SESSION['message']; //display the message
		    unset($_SESSION['message']); //free it up
		}
		?>
		<?php //styles for this page ?>
		<style>
			#zanders_content{
				width:62%; 
				/*margin-right:2%;*/
				float:left;
				background-color:#fff;
				border: 1px solid #e5e5e5;  
				-webkit-box-shadow: 0 1px 1px rgba(0,0,0,.04);  
				box-shadow: 0 1px 1px rgba(0,0,0,.04);
				padding:10px;
				margin-bottom:10px;
			}
			#zanders_aside{
				width:35%;
				float:right;
			}
			.zanders_box{
				position: relative;  
				min-width: 255px;  
				border: 1px solid #e5e5e5;  
				-webkit-box-shadow: 0 1px 1px rgba(0,0,0,.04);  
				box-shadow: 0 1px 1px rgba(0,0,0,.04);  
				background: #fff;  
				margin-bottom: 20px;  
				padding: 0;  
				line-height: 1;
			}
			.zanders_box h3{
				font-size: 14px;  
				padding: 8px 12px;  
				margin: 0;  
				line-height: 1.4;
			}
			.zanders_inside{
				margin: 11px 0;  
				position: relative;
				padding: 0 12px 12px;  
				line-height: 1.4em;  
				font-size: 13px;
			}
			.field.go{
				width: 100%;  
				text-align: right;
			}
			.field.go .submit{
				text-align: right;
				padding:0px;
			}

			#sortable1, #sortable2 {
			  border: 1px solid #eee;
			  width: 282px;
			  min-height: 20px;
			  list-style-type: none;
			  margin: 0;
			  padding: 5px 0 0 0;
			  float: left;
			  overflow-y:scroll;
			  overflow-x:hidden;
/*			  margin-right: 10px;

*/			}
			#sortable1 li, #sortable2 li {
			  margin: 0 5px 5px 5px;
			  padding: 5px;
			  font-size: 0.9em;
			  width: 242px;
			  cursor:move;
			}
		</style>

		<form name="download_files_form" method="post" action="/wp-admin/admin-post.php">
			
		<div style="margin-top:20px;">
			<div id="zanders_aside">
				<div id="cron" class="zanders_box"> 
  					<h3>Cron Job Settings</h3>
  					<div class="zanders_inside">
	  					<p>The cron settings are currently 15 minutes, the buttons here will add/remove this from the wordpress cron. </p>
	  					<p>
	  						To schedule an import, you must create a cron job in your web hosting control panel. If you plan to import all the products every day you will have to follow the guide 
	  						<a href="/wp-admin/admin.php?page=pmxi-admin-manage&id=1&action=scheduling">located here</a>. 
	  					</p>
	  					<div class="field go">
							<!--activate_zanders_cron-->
							<input type="button" id="btnCronDisable" value="Disable Cron" <?php echo !zanders_cron_enabled() ? "disabled=disabled style='display:none;'" : ""; ?>>
							<input type="button" id="btnCronEnable" value="Enable Cron" <?php echo zanders_cron_enabled() ? "disabled=disabled style='display:none;'" : ""; ?>>
	  					</div>
  					</div>
				</div>
				<div id="files" class="zanders_box"> 
  					<h3>File Selection</h3>
  					<div class="zanders_inside">
	  					<p>This is where you select the files that you want to download when the cron job runs.</p>
	  					<p><input type="button" id="checkftp" value="Connect and Choose Files"></p>

  						<div style="position:relative;z-index:5;display:block;">
	  						<div class="overlay1" style="position:absolute;width:100%;height:100%;z-index:10;display:none;top:0px;left:0px;"></div>

	  						<div style="float:left;">
	  							Remote Files<br />
	  							<ul id="sortable1" class="connectedSortable" style="min-height:300px;background-color:#fff;"></ul>
	  						</div>
	  						<div style="float:right;">
	  							Files to Download<br />
	  							<ul id="sortable2" class="connectedSortable" style="min-height:300px;background-color:#fff;">
	  							<?php 
	  								$files = get_option("zanders_file_list"); 
	  								if($files):
	  								foreach($files as $file):
	  									echo "<li class='ui-state-default'>{$file}</li>";
	  								endforeach;
	  								endif;
	  							?>
	  							</ul>
	  						</div>
	  						<div style="clear:both;"></div>
  						</div>
	  					<div class="field go" style="margin-top:10px;">
							<input type="button" id="btnDownload" value="Download Selected Files Now" style="float:right;">
	  					</div>
  						<div style="clear:both;"></div>

  					</div>
				</div>
			</div>			
			<div id="zanders_content">
				<h2>File Transfer Utility</h2>
				<em>Please note: The file settings here will also be the ones that are used in the automatic downloading.</em><br />
			</div>
			<div id="zanders_content">
				<h2>Directory Settings</h2>
				<table class="form-table">
					<tr valign="top">
						<th scope="row">Local Directory</th>
						<em><strong>NOTE: if you do not use the default directory and are using the main product import in wp-allimport - the URL IN WPALLIMPORT MUST MATCH</strong></em>
						<td>
							<input type="text" name="zanders_ftp_local_folder" id="zanders_ftp_local_folder" value="<?php echo get_option("zanders_ftp_local_folder"); ?>" placeholder="/wp-content/uploads/zandersftp/">
							<em>leave blank to use the default</em>
						</td>
					</tr>
			        <tr valign="top">
			        	<th scope="row">Remote Directory</th>
			        	<td>
			        		<input type="text" name="zanders_ftp_remote_folder" id="zanders_ftp_remote_folder" value="<?php echo get_option("zanders_ftp_remote_folder"); ?>" placeholder="/Inventory/">
			        		<em>leave blank to use the default</em>
			        	</td>
			        </tr>
				</table>
				<div class="field go" style="float:right;">
				<?php submit_button( 'Update Directory Settings', 'primary', 'submit' ) ?>
				</div>

			</div>

			<div id="zanders_content">
				<h2>Download Status
				</h2>
				<div id="downloadstatus" style="position:relative;min-height:100px;min-width:100px;">
					<div class="overlay" id="dlstatusoverlay" style="position:absolute;width:100%;height:100%;z-index:10;display:none;"></div>
					<div class="downloadfilescontainer"></div>
				</div>
			</div>


		</div>
		<div style="clear:both;"></div>
			  <link rel="stylesheet" href="//code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css">
			  <script src="//code.jquery.com/ui/1.11.4/jquery-ui.js"></script>
			  <script>
			  jQuery(function($) {
			  		$("#checkftp").click(function(){
			  			$(".overlay1").fadeIn();

	                jQuery.ajax({
		                url: "<?php echo admin_url('admin-ajax.php'); ?>",
		                type: 'POST',
		                data: {
		                    action: 'list_remote_files',
		                    server: '<?php echo get_option('zanders_ftp_server'); ?>',
		                    port: '<?php echo get_option('zanders_ftp_port'); ?>',
		                    username: '<?php echo get_option('zanders_ftp_username'); ?>',
		                    password: '<?php echo get_option('zanders_ftp_password'); ?>',
		                    remotefolder: '<?php echo get_option('zanders_ftp_remote_folder'); ?>',
		                },
		                dataType: 'json',
		                success: function(response) {
		                    //console.log(response);
	                    	$("#sortable1").html("");
	                    	$.each(response, function(i, item){
	                    		if(item.indexOf(".") > 0 ){
	                    			if( $("#sortable2").children("li:contains(" + item + ")").length < 1 ){ // check for duplicates
	                    				$("#sortable1").append("<li class='ui-state-default'>" + item + "</li>");
	                    			}
	                    		}
	                    	});
	                    	$(".overlay1").fadeOut();
		                }
			    	});
				});

			    $( "#sortable1, #sortable2" ).sortable({
			      connectWith: ".connectedSortable",
			    }).disableSelection();
			    //only updating the list of files that we want to keep
			    $( "#sortable2" ).sortable({
			      	update : function(){
			      		$(".overlay1").fadeIn();

			      		//save the array to the database here
		      			var filelist = new Array();
		      			$("#sortable2 li").each(function(){
		      				var file = $(this).text();
		      				filelist.push(file);
		      			});
		      			$.post("<?php echo admin_url('admin-ajax.php'); ?>",{'action': 'update_filelist', 'filelist': filelist},function(data){
	      					//console.log(data);
		                    $(".overlay1").fadeOut();

		      			}, "json");
		      		}
			    });

			  });
				jQuery("#btnDownload").click(function(){
					jQuery("#dlstatusoverlay").fadeIn();
					var i = 0;
					var x = 0;
					jQuery("#sortable2 li").each(function(){i++;});
					//console.log(i);

					jQuery("#sortable2 li").each(function(){
						var file = jQuery(this).text();
						
						jQuery("#downloadstatus .downloadfilescontainer").empty();
		      			
		      			jQuery.post("<?php echo admin_url('admin-ajax.php'); ?>",{'action': 'download_file', 'file': file},function(data){
		  					//console.log(data);
							x++;
							//console.log(x);
		  					jQuery("#downloadstatus .downloadfilescontainer").append("" + data + "");
	  						if(x == i){
  								jQuery("#dlstatusoverlay").fadeOut();			
	  						}
		      			});

	      			});
				});
				<?php // jquery for the cron buttons ?>
				jQuery("#btnCronDisable").click(function(){
					jQuery.post("<?php echo admin_url('admin-ajax.php'); ?>",{'action': 'deactivate_zanders_cron'},function(data){
						jQuery("#btnCronDisable").prop("disabled",true).fadeOut(function(){
							jQuery("#btnCronEnable").prop("disabled",false).fadeIn();
						});
					});
				});
				jQuery("#btnCronEnable").click(function(){
					jQuery.post("<?php echo admin_url('admin-ajax.php'); ?>",{'action': 'activate_zanders_cron'},function(data){
						jQuery("#btnCronEnable").prop("disabled",true).fadeOut(function(){
							jQuery("#btnCronDisable").prop("disabled",false).fadeIn();
						});
					});
				});

			</script>


		 <?php wp_nonce_field( 'check_download_files', 'check_download_files' ); ?>
		<input type="hidden" name="action" value="update_custom_settings">
		<input type="hidden" name="command" value="update_download_files">
		<input type="hidden" name="returl" value="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">


		<?php //submit_button( 'Update Settings', 'primary', 'submit' ) ?>
	</form>



	<?php  //  elseif($active_tab == 'import_products'): // future use?>

<?php endif; ?>
</div>
<?php
}